...
exports.handler = function(event, context, callback) {
    console.log("Device_id:" + event.device_id.toString() + "Humidity:" + event.humidity.toString());
    if (event.humidity > 50) {
        var postData = JSON.stringify({ time: event.time, command: "automated dehumidifier will be switched on" });
        var params = {
            topic: 'my_topic',
            payload: postData,
            qos: 0
