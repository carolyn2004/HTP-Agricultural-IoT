...
exports.handler = function(event, context, callback) {
    console.log("time:" + event.time + "Humidity: " + event.humidity.toString());
    if (event.humidity < 50) {
        var postData = JSON.stringify({ time: event.time, command: "automated humidifier will be switched on" });
        var params = {
            topic: 'my_topic',
            payload: postData,
            qos: 0
